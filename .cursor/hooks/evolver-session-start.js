...
function loadMemoryPathFromUserEnvironment() {
  if (process.env.MEMORY_GRAPH_PATH && fs.existsSync(process.env.MEMORY_GRAPH_PATH)) return;
  ...
  if (process.platform === 'win32') {
    try {
      const result = require('child_process').spawnSync('powershell', [
        '-NoProfile',
        '-Command',
        '[Environment]::GetEnvironmentVariable("MEMORY_GRAPH_PATH", "User")',
      ], {
        encoding: 'utf8',
        timeout: 3000,
        shell: false,
      });
      const userPath = String(result.stdout || '').trim();
      if (userPath) process.env.MEMORY_GRAPH_PATH = userPath;
    } catch { /* best-effort */ }
  }

  if (!process.env.MEMORY_GRAPH_PATH) {
    const memoryPath = path.join(os.homedir(), '.evomap', 'memory', 'evolution', 'memory_graph.jsonl');
    try {
...
function main() {
  loadMemoryPathFromUserEnvironment();
